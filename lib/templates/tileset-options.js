...
import { updateFile, updatePreview } from '../stores/tileset/actions';
...
export default function TilesetOptions({ store, visible }) {
  return m('.main', [
    m('div.content.options', { class: (visible ? '' : 'hidden') }, [
      TextInput({
        label: 'Path to image',
        value: store.getState().fileContent.image,
        onChange: value => store.dispatch(updateFile({ image: value })),
        suffix: m('button.btn', { onclick: () => store.dispatch(updatePreview()) }, 'Update')
      }),
